...
# See also https://github.com/Detlew/PowerTOST by Detlew Labes, Helmut Schuetz, Benjamin Lang
...
function owensqo(nu::Real, t::Real, delta::Real, b::Real; a::Real = 0.0)::AbstractFloat
    ...
    if nu%2 > 0
        ...
        qv = cdf(ZDIST, -delta) + sqrt(2π) * sumt
    end
    #
end
...
@inline function tfn(x, fx)
    ...
    if tv3 <= (log(1 + fxs) - xs * fxs)
        ...
        while true
            rt  = fxs + 1.0
            x2  = x1 + (xs * fxs + tv3 - log(rt)) / (2x1 * (1 / rt - xs))
            fxs = x2 * x2
